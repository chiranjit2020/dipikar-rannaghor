...
const STATUS_STYLE: Record<TaskStatus, string> = {
  backlog: 'text-ink-muted border-tint/10 bg-tint/[0.03]',
...
const PRIORITY_STYLE: Record<Priority, string> = {
  ...
  low: 'text-ink-muted border-tint/10 bg-tint/[0.03]',
};
...
export function PhaseBadge({ code }: { code: string }) {
  return (
    <span className="inline-flex items-center rounded-full border border-hairline bg-tint/[0.03] px-2.5 py-0.5 text-xs font-medium text-ink-soft">
      {code}
    </span>
  );
}
...
export function KindBadge({ kind }: { kind: string }) {
  return (
    <span className="inline-flex items-center rounded-md border border-hairline bg-tint/[0.04] px-1.5 py-0.5 text-[0.7rem] font-semibold uppercase tracking-wide text-ink-muted">
